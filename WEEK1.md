...
IdentityFile ~/.ssh/id_rsa
  ...
  ** Clone repository at Gitbash **
    ...
    * `pwd` -- Check and verify that the current working directory is C:\users\Hvuser/workspace
    * `git clone ssh://git-codecommit.us-east-2.amazonaws.com/v1/repos/FTP117 `
    * `cd FTP117`
    * `git status`
  * open Visual Studio (VS) Code
    * Open folder c:\users\Hvuser\workspace\FTP117
...
# Workshop material - Day #2 - MySQL Workbench/Cli/Curl/Browser to interact w/ application
  ...
  * `CREATE DATABASE FTP117;` and click the lightning button
  * `CREATE USER 'FTP117'@'localhost' IDENTIFIED BY 'FTP117';`
  * `GRANT ALL ON FTP117.* TO 'FTP117'@'localhost';`
  ...
  * go to gitbash, ensure you are in workspace/FTP117
  * `cd restservice/leavemanager`
  * `mvn compile`
  * `mvn exec:java -Dexec.mainClass=com.hexaware.FTP117.util.CliMain`
  ...
  * Build the war file as follows:
    ...
    * `cp target/FTP117-0.0.1-SNAPSHOT.war /D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16/webapps/FTP117.war`
    * start tomcat and tail its logs
      * `cd D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16`
      * `rm -rf logs/*`
      * `./bin/startup.sh`
      * `tail -f logs/*`
      * `curl -vvv http://localhost:8080/FTP117/api/employees | python -m json.tool`
      * `curl -vvv http://localhost:8080/FTP117/api/employees/2000 | python -m json.tool`
   * `cd ../../webui/lm-app/`
   * `npm install`
   * `ng build`
   * `cp ../../restservice/leavemanager/target/FTP117-0.0.1-SNAPSHOT.war ./FTP117.war`
   * `cd dist`
   * `jar -uvf ../FTP117.war *`
   * `cd ..`
   * `jar -tvf FTP117.war`
   * `cp FTP117.war /D/FTP/apache-tomcat-8.5.16-windows-x64/apache-tomcat-8.5.16/webapps/FTP117.war`
   * Notice that the tail terminal shows that the new version of the web application archive is now getting deployed
   * Open Chrome browser and navigate to http://localhost:8080/FTP117/
...
# Workshop material - Day #4 - Configure Project Board in Trello for  Managing and Tracking Your Project
  ...
  * Create a board for your project as "FTP117"
  ...
  * [Team member #3]
     * Create "Current Sprint" with "FTP117 Sprint 1" as label 
